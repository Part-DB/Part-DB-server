...
### constants ###
STAGEFILE="Part-DB-release-stage"
...
LOCALVERSION="$PRODENV/VERSION"

DBUSER="${1-""}"
DBPASSWD="${2-""}"

SCRIPTDIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
STAGEFILE="$SCRIPTDIR/$STAGEFILE"

### variables ###
stage="1"

echo ""
echo "*** Part-DB upgrade tool v0.001 (WorksForMe edition) ***"
...
# (allow empty password)
	echo "usage: $0 database_username database_password"
	exit -2
fi
...
#               ln -sf $WEB_AVAILABLE/partdb_testing.conf $WEB_ENABLED/partdb_testing.conf
        ...
        if [ "$stage" = "2" ]; then
                ...
                # cd into working dir
                cd $GITREPO
                chown -R root:root $GITREPO
                git fetch --tags
                # get latest version
                version=$(git describe --tags --abbrev=0)
		now=$(date -Iseconds)
                # dump DB, preventing overwrite by re-execution if e.g. the migration broke the database structure
                mysqldump -u$DBUSER -p$DBPASSWD partdb > "$BACKUPDIR/partdb_before_update_${version}_${now}.sql"
